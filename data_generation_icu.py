...
class Generator():
    ...
    def generate_adm(self):
        ...
        #Limit split to the expected number of components, filling any missing values
        data[['hours', 'min', 'sec']] = data['hours'].str.split(':', expand=True, n=2)
...
#         print("# Unique insurance",self.data.insurance.nunique())
        ...
        for hid in self.hids:
            ...
            dataDic[hid] = {'Cond': {},
                            ...
                            'age': int(grp['Age'].iloc[0]),  # Ensure single value extraction
                            'gender': grp['gender'].iloc[0],
                            'label': int(grp['label'].iloc[0])  # Ensure single value extraction
                           }

        ...
        for i in range(self.k_fold):
            ...
            for epoch in range(args.num_epochs):
                ...
                for nbatch in range(int(len(train_hids)/(args.batch_size))):
                    #print(nbatch)
                    meds,chart,out,proc,lab,stat_train,demo_train,Y_train, edge_index=self.getXY(train_hids[nbatch*args.batch_size:(nbatch+1)*args.batch_size],labels)
#                     
                    #print(f"Batch {nbatch} meds shape:", meds.shape)
                    #print(f"Batch {nbatch} labels shape:", Y_train.shape)
                    ...
                    #extra start
                    # Compute loss
                    batch_loss = self.loss(output, Y_train, logits, train=True)
                    ...
                    # Accumulate loss and other stats
                ...
                print(f"Epoch {epoch:.1f}, Training Loss: {epoch_loss:.4f}")
                
                #self.loss(output, Y_train, logits, False, False)
                #self.loss(output, Y_train, logits, True, False)
    ...
    def model_val(self,val_hids):
        ...
        for nbatch in range(int(len(val_hids)/(args.batch_size))):
            meds,chart,out,proc,lab,stat_train,demo_train,y, edge_index=self.getXY(val_hids[nbatch*args.batch_size:(nbatch+1)*args.batch_size],labels)
        ...
        print("val_prob shape:", torch.tensor(val_prob).shape)
        print("val_truth shape:", torch.tensor(val_truth).shape)
        print("val_logits shape:", torch.tensor(val_logits).shape)

        
        
        # Convert to tensors
        val_prob = torch.tensor(val_prob)
        val_truth = torch.tensor(val_truth)#.squeeze()
        val_logits = torch.tensor(val_logits)

        #self.loss(val_prob,val_truth,val_logits,False,False)
        val_loss = self.loss(val_prob, val_truth, val_logits, False, False)
        print(f"Loss Value: {val_loss}")
    ...
    def model_test(self,test_hids):
        ...
        for nbatch in range(int(len(test_hids)/(args.batch_size))):
            meds,chart,out,proc,lab,stat,demo,y, edge_index=self.getXY(test_hids[nbatch*args.batch_size:(nbatch+1)*args.batch_size],labels)
            
            output,logits = self.net(meds,chart,out,proc,lab,stat,demo, edge_index)
            #y = y.unsqueeze(1) 
            # Debugging shapes
...
#            
        ...
        # Convert to tensors and ensure shapes match
        prob_tensor = torch.tensor(self.prob)#.unsqueeze(1)
        truth_tensor = torch.tensor(self.truth)#.view(-1)  # Flatten to match logits
        logits_tensor = torch.tensor(self.logits)

        #print("Prob tensor shape:", prob_tensor.shape)
        #print("Truth tensor shape:", truth_tensor.shape)
        #print("Logits tensor shape:", logits_tensor.shape)

        # Call loss function
        self.loss(prob_tensor, truth_tensor, logits_tensor, train=False)
      
        #self.loss(torch.tensor(self.prob),torch.tensor(self.truth).squeeze(),torch.tensor(self.logits),False,False)
...
#             print(stat_df)    
            ...
            #demo = demo[-self.batch_size:]  # Trims to a fixed batch size
        ...
        demo_np= demo_df.numpy()
        similarity_matrix= cosine_similarity(demo_np)

        # Define edges based on a similarity threshold
        threshold = 0.8  # Define a similarity threshold
        edges = np.argwhere(similarity_matrix > threshold)  # Find similar pairs
        edge_index = torch.tensor(edges.T, dtype=torch.long) 

        
        # Debug output
        return meds,chart,out,proc,lab ,stat_df, demo_df, y_df , edge_index      
            
           
        
    
    
    def train_model(self,meds,chart,out,proc,lab,stat_train,demo_train,Y_train, edge_index):
        ...
        print("Logits shape:", logits.shape)
        # Squeeze tensors if needed
        output = output.squeeze()
        logits = logits.squeeze()
        Y_train = Y_train.view(-1, 1) # Ensure Y_train is [batch_size]
        # Compute loss
        if logits.shape[0] != Y_train.shape[0]:
            raise ValueError(f"Mismatch in shapes: Logits shape: {logits.shape}, Y_train shape: {Y_train.shape}")
    
        out_loss = self.loss(output, Y_train, logits, train=True, standalone=False)
    ...
    def create_model(self,model_type):
        if model_type=='Time-series LSTM':
            ...
            self.net = model.CNNBaseH(self.device,
                               ...
                               embed_size=args.embedding_size,rnn_size=args.rnn_size,
                               batch_size=args.batch_size) 
        elif model_type == 'LSTM Attention GNN':
            self.net = model.LSTMWithAttention_GNN(self.device,
                               self.cond_vocab_size,
                               self.proc_vocab_size,
                               self.med_vocab_size,
                               self.out_vocab_size,
...
#         print(self.gender)
        ...
        # Trim all arrays to the target length
        self.eth = self.eth[:target_length]
        self.gender = self.gender[:target_length]
        self.age = self.age[:target_length]
        self.ins = self.ins[:target_length]
        self.prob = self.prob[:target_length]
        self.logits = self.logits[:target_length]
        self.truth = self.truth[:target_length]

...
class Loss(nn.Module):
    ...
    def forward(self, prob, labels,logits, train=True, standalone=False):
        ...
        # Ensure the shapes match for indexing
        if prob.dim() == 1:  # If prob is 1D, add a second dimension
            prob = prob.unsqueeze(1)
        if labels.dim() == 1:  # If labels is 1D, add a second dimension
            labels = labels.unsqueeze(1)
        
        pos_ind = labels >= 0.5
        neg_ind = labels < 0.5
        #pos_ind = (labels == 1).nonzero(as_tuple=True)[0]
        #neg_ind = (labels == 0).nonzero(as_tuple=True)[0]
        pos_label = labels[pos_ind]
        neg_label = labels[neg_ind]

        # Ensure indices are within range of prob
        #if prob.shape[0] < max(pos_ind) or prob.shape[0] < max(neg_ind):
         #   raise ValueError("Mask indices are out of range for prob")
...
#             print(prob)
    ...
    def auroc_plot(self,label, pred):
        ...
        plt.plot(fpr, tpr, label=f'LSTM With Attention- GNN, auc = {str(round(auc,3))}')

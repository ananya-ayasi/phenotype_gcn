...
#             print(chartEmbed[5,:,0:10])
        ...
        min_batch_size = min(out2.size(0), gender.size(0))
        # Trim both tensors to the minimum batch size
        out2 = out2[:min_batch_size]
        gender = gender[:min_batch_size]
        # Concatenate along dimension 1
        out2 = torch.cat((out2, gender), dim=1)
...
#         print(gender.shape)
        ...
        # Determine the minimum batch size
        min_batch_size = min(out2.size(0), eth.size(0))
        # Trim both tensors to the minimum batch size
        out2 = out2[:min_batch_size]
        eth = eth[:min_batch_size]
        # Concatenate along dimension 1
        out2 = torch.cat((out2, eth), dim=1)
...
#         print(eth.shape)
        ...
        # Determine the minimum batch size
        min_batch_size = min(out2.size(0), ins.size(0))

        # Trim both tensors to the minimum batch size
        out2 = out2[:min_batch_size]
        ins = ins[:min_batch_size]

        # Concatenate along dimension 1
        out2 = torch.cat((out2, ins), dim=1)
...
#         print(ins.shape)
        ...
        age=age.to(self.device)
        # Determine the minimum batch size
        min_batch_size = min(out2.size(0), age.size(0))

        # Trim both tensors to the minimum batch size
        out2 = out2[:min_batch_size]
        age = age[:min_batch_size]

        # Concatenate along dimension 1
        out2 = torch.cat((out2, age), dim=1)
